...
<!-- startPanier -->
	...
	<tfoot>
		<tr>
			...
			<td>&nbsp;</td>
			<td id="prixTotal">
		    	<c:out value="${location.prixLocFormate}"/>
		    </td>
		    <td>&nbsp;</td>
		</tr>
	</tfoot>
	<form name="louerPanier" action="/${ROOT}/service/location.htm" method="post">
	   	...
	   	<c:forEach var="produit" items="${location.mesProduits}" varStatus="i">
			...
			<tr>
				...
				<td><c:out value="${produit.prixLocFormate}"/></td>
				<td><span id="totalProduit${i.index}"><c:out value="${produit.prixLocTotalFormate}"/></span></td>
				<td>
		...
		<c:if test="${empty location.mesProduits}" >
		<tr>
