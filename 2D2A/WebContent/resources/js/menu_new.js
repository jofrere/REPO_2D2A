...
//----------------------------------------------------------------------------------------------------
  ...
  this.entry = function(level, height, text, url, target, onClick, id, formLink) {
    var nr = this.items.length;
    var parent = nr - 1;
    this.items[nr] = new makeItem(level, height, text, url, target, onClick, id, formLink);
    if(parent >= 0) {
    ...
    for(var i = cnt = 0; i < this.items.length; i++) {
      if(this.items[i].parent == parent) {
        ...
        document.write('<table border=0 cellspacing=0 cellpadding=' + itemPadding +
                       ...
                       '<td id="text' + this.mNr + '_' + i + '" class="' + clsItem + '" align="' + itemAlign + '"><form action="' +  this.items[i].formLink + '" method="POST"><input type="hidden" name="RUBRIQUE" value="' + this.items[i].id + '" /></form>' +
...
function makeItem(level, height, text, url, target, onClick, id, formLink) {
  this.level = level;
  this.height = height;
  this.text = text;
  this.url = url;
