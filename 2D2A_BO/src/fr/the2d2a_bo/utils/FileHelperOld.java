...
public class FileHelperOld {
	
	private static Logger logger = Logger.getLogger(FileHelperOld.class);
	...
	public static String writeFile(String path, MultipartFile file) {
		try {
			...
			String newFileName = FileHelperOld.createFileName(fileName);
	...
	public static String manageUploadedFile(MultipartFile file, String oldFile, String path, String pathServer) {
		if (file.isEmpty()) {
			...
			boolean fileDeleted = FileHelperOld.deleteFile(path, oldFile);
			logger.debug("File deleted ? " + fileDeleted);
			
			String fileGenerated = FileHelperOld.writeFile(path, file);
